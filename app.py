...
# Landmarks data with complete descriptions and coordinates
LANDMARKS = [
    {
        "id": 1,
...
# Create map function
def create_landmark_map(landmark):
    m = folium.Map(
...
def show_landmark_modal(landmark):
    modal_html = f"""
    <div class="modal-overlay" onclick="if(event.target === this) window.parent.location.hash = '';">
        <div class="modal-content">
            <div class="modal-header">
                <button class="modal-close" onclick="window.parent.location.hash = '';">✕</button>
                <div class="modal-category">{landmark['type']}</div>
                <h2 class="modal-title">{landmark['title']}</h2>
            </div>
            <div class="modal-body">
                <p class="modal-description">{landmark['description']}</p>
                <div class="modal-date">🕐 {landmark['date']}</div>
            </div>
        </div>
    </div>
    """
...
# Modal Popup for Landmark Details
if st.session_state.selected_landmark:
    ...
    # Create two tabs: Story and Map
    tab1, tab2 = st.tabs(["📖 Story", "📍 Map"])
    ...
    with tab1:
        ...
        st.markdown('<div class="divider-line"></div>', unsafe_allow_html=True)
        st.markdown(f"<div class='modal-date'>🕐 {landmark['date']}</div>", unsafe_allow_html=True)
    
    with tab2:
        st.markdown(f"## 📍 {landmark['title']} Location")
        
        # Create and display map
        m = create_landmark_map(landmark)
        
        # Display map using streamlit_folium
        st_folium(m, width=700, height=500)
        
        st.info(f"📌 **Location**: {landmark['title']}\n\nCoordinates: {landmark['coordinates'][0]:.4f}°N, {abs(landmark['coordinates'][1]):.4f}°W")
